...
import { Home, Users, FileText, PieChart, Settings, HelpCircle, MessageSquare, LogOut, Shield, ChevronRight, BookOpen } from 'lucide-react';
      ...
      <TooltipTrigger asChild>
        <li className="mb-2">
          {onClick ? <Button variant="ghost" className={cn("flex w-full items-center gap-2 px-3 py-2.5 rounded-lg cursor-pointer transition-all justify-start font-normal", isActive ? "bg-primary/10 text-primary" : "text-muted-foreground")} onClick={onClick}>
            ...
            </Button> : <Link to={to} className={cn("flex items-center gap-2 px-3 py-2.5 rounded-lg cursor-pointer transition-all", collapsed ? "justify-center" : "", isActive ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-secondary hover:text-foreground")}>
              <div className="w-5 h-5 flex items-center justify-center">
                {icon}
              </div>
              {!collapsed && <span className="ml-1">{label}</span>}
...
export function Sidebar({
  ...
  return <div className={cn("h-screen fixed left-0 top-0 flex flex-col bg-sidebar border-r border-border", collapsed ? "w-20" : "w-60", "transition-all duration-300 ease-in-out z-20", className)}>
      <Button variant="ghost" size="icon" className="absolute -right-3 top-8 bg-background border border-border rounded-full hover:bg-secondary z-10" onClick={() => setCollapsed(!collapsed)}>
      ...
      <nav className="flex-1 px-3 py-4 overflow-y-auto scrollbar-none">
        <ul className="space-y-1">
          ...
          <NavItem icon={<BookOpen size={18} />} label="Documentação" to="/docs" isActive={location.pathname === '/docs'} collapsed={collapsed} />
