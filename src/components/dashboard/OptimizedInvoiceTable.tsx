...
import { usePaginatedInvoices } from '@/hooks/use-paginated-invoices';
...
interface OptimizedInvoiceTableProps {
  onEditInvoice?: (invoiceId: string) => void;
}

export function OptimizedInvoiceTable({ onEditInvoice }: OptimizedInvoiceTableProps) {
  ...
  // Use the optimized paginated hook
  const {
    invoices,
    totalCount,
    totalPages,
    isLoading,
    refetch
  } = usePaginatedInvoices(currentPage, itemsPerPage, {
  ...
  return (
    <div className="bg-gradient-to-br from-gray-900/50 to-gray-800/30 border border-white/10 rounded-2xl overflow-hidden backdrop-blur-sm">
      ...
      {!isLoading && totalCount > 0 && (
        <div className="py-4 px-6 border-t border-white/10 flex justify-between items-center">
          <div className="text-sm text-gray-400">
            Mostrando {Math.min(itemsPerPage, totalCount)} de {totalCount} faturas
          </div>
